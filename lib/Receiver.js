...
Receiver.prototype.processPacket = function (data) {
  if ((data[0] & 0x30) != 0) {
    this.error('reserved 2 and 3 fields must be empty', 1002);
    return;
  }
    ...
    finish: function(mask, data) {
      ...
      if (this.state.lastFragment) {
        ...
        if(this.state.compressed) {
          ...
          zlib.inflateRaw(bc, function(err, messageBuffer) {
            if (self.dead) {
              // connection was closed while inflating, so don't do anything
              return;
            }
            if(err) {
            ...
            self.ontext(messageBuffer.toString('utf8'), {
              ...
              compressed: true,
              compressed_size: buf.length,
              compression: 1.0 - (buf.length / messageBuffer.length)
            });
          });
        } else {
          ...
          self.ontext(buf.toString('utf8'), {masked: this.state.masked, buffer: buf});
        }
      }
      this.endPacket();
    }
    ...
    finish: function(mask, data) {
      ...
      if (this.state.lastFragment) {
        ...
        if(this.state.compressed) {
          ...
          this.onbinary(buf, {masked: this.state.masked, buffer: buf});
        }
      }
      this.endPacket();
    }
